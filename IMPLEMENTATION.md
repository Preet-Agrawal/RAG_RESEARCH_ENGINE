...
*(Housekeeping note: the raw `results/*.json` files for the 6 attempts below were deleted in a later cleanup pass — every number, config, and error message they contained is quoted verbatim in the prose here, so nothing was lost, but the specific filenames referenced below (`..._014527.json` etc.) no longer exist on disk. For the follow-up experiments that did produce a clean, citable result, see `docs/PARA_EMPIRICAL_FINDINGS.md`, whose backing JSON files are still present in `results/`.)*

**Attempt 1** (formerly `ablation_component_stack_20260708_014527.json`) — `overall_accuracy` came back **100% → 100% → 57% → 0%** as position correction was progressively enabled, which reads exactly like the ablation "proving" position correction *hurts* retrieval. It doesn't. `semantic_mean` was exactly `0.000` on every failing run — the signature of `build_para_context()`'s empty-result fallback (`return "", 0.0`), which only happens when the whole `apply_para()` call raised and got swallowed by `run_one_config()`'s bare `except Exception: answer_text = f"[ERROR] {e}"`. The script recorded "needle not found" for what was actually an unlogged crash. **The original `run_ablation.py` had no way to tell the two apart.**
